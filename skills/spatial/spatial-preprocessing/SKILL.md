...
```bash
# Basic usage
python skills/spatial/spatial-preprocessing/spatial_preprocessing.py \
...
# Demo
python skills/spatial/spatial-preprocessing/spatial_preprocessing.py --demo --output /tmp/demo

# Via CLI (using 'spatialclaw run' or 'python spatialclaw.py run')
spatialclaw run spatial-preprocessing --input <file> --output <dir>
...
| `--data-type` | `auto` | Input data type/platform hint |
| `--species` | `human` | Species for mitochondrial gene heuristics |
| `--tissue` | - | Optional tissue QC preset |
| `--min-genes` | preset | Minimum genes per spot/cell |
| `--min-cells` | preset | Minimum cells/spots per gene |
| `--max-mt-pct` | `20.0` | Maximum mitochondrial percentage |
| `--max-genes` | preset | Maximum genes per spot/cell |
| `--n-top-hvg` | `2000` | Number of highly variable genes |
| `--n-pcs` | `30` | Number of PCA components |
| `--n-neighbors` | `15` | Number of neighbors for graph construction |
| `--leiden-resolution` | `0.5` | Default Leiden clustering resolution |
| `--resolutions` | - | Comma-separated Leiden resolutions for exploration |
