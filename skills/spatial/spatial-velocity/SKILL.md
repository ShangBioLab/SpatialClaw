...
|-----------|---------|-------------|
| `--input` | (required unless `--demo`) | AnnData file with `spliced` and `unspliced` layers |
| `--output` | `results/spatial_velocity` | Output report directory |
| `--demo` | off | Run the built-in velocity demo |
| `--method` | `stochastic` | `stochastic`, `deterministic`, `dynamical`, or `velovi` |
...
**Chaining partners**:
- `spatial-preprocessing` — QC before velocity calculations
