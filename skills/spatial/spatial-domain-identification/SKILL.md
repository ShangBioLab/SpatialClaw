...
spatialclaw run spatial-domain-identification --input <preprocessed.h5ad> --output <report_dir>

# Specify method and parameters
spatialclaw run spatial-domain-identification \
...
# Demo mode
spatialclaw run spatial-domain-identification --demo --output /tmp/domains_demo

# Note: use 'spatialclaw run' or 'python spatialclaw.py run'
python spatialclaw.py run spatial-domain-identification --demo
```
...
|-----------|---------|-------------|
| `--input` | (required unless `--demo`) | Preprocessed AnnData input |
| `--output` | `results/spatial_domain_identification` | Output report directory |
| `--demo` | off | Run the built-in demo |
| `--method` | `leiden` | `leiden`, `louvain`, `spagcn`, `stagate`, `graphst`, or `banksy` |
| `--n-domains` | auto | Target number of domains for methods that support it |
| `--resolution` | method-specific | Leiden/Louvain/BANKSY clustering resolution |
| `--spatial-weight` | `0.3` | Weight assigned to spatial adjacency for graph clustering |
| `--rad-cutoff` | auto | Radius cutoff for SpaGCN/STAGATE-style spatial graphs |
| `--k-nn` | `6` | Spatial nearest-neighbor count for STAGATE graph construction |
| `--lambda-param` | `0.2` | BANKSY neighborhood/expression weighting parameter |
| `--refine` | off | Apply spatial label refinement after clustering |
...
- `--spatial-weight`: Weight of spatial graph (0.0-1.0, default 0.3)
- `--rad-cutoff`: SpaGCN spatial radius cutoff.
- `--lambda-param`: BANKSY neighborhood/expression weighting parameter.
...
**Key parameters**:
- `--n-domains`: Target number of spatial domains
...
**Key parameters**:
- `--n-domains`: Target number of domains
- `--k-nn`: Nearest neighbors for spatial topology graph (default 6)
...
**Key parameters**:
- `--lambda-param`: Spatial regularization (default 0.2)
- `--n-domains`: Exact cluster quantity target (optional)
...
**Chaining partners**:
- `spatial-preprocessing`: Provides the preprocessed h5ad input
- `spatial-de`: Downstream differential expression between domains
- `spatial-enrichment`: Gene set enrichment per domain
- `spatial-cell-communication`: Cell-cell communication across domain boundaries
