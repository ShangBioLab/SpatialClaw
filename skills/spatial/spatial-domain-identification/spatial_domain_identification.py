#!/usr/bin/env python3
"""Spatial domain identification for tissue regions and spatial niches.
...
Usage:
    python spatial_domain_identification.py --input <preprocessed.h5ad> --output <dir>
    python spatial_domain_identification.py --demo --output <dir>
    python spatial_domain_identification.py --input <file> --method spagcn --n-domains 7 --output <dir>
"""
...
from skills.spatial._lib.domain_identification import (
    SUPPORTED_METHODS,
    dispatch_method,
    refine_spatial_domain_identification,
)
...
SKILL_NAME = "spatial-domain-identification"
SKILL_VERSION = "0.4.0"
...
def generate_figures(adata, output_dir: Path) -> list[str]:
    ...
    if spatial_key is not None:
        try:
            ...
            p = save_figure(fig, output_dir, "spatial_domain_identification.png")
...
def write_report(output_dir: Path, summary: dict, input_file: str | None, params: dict) -> None:
    ...
    cmd = f"python spatial_domain_identification.py --input <input.h5ad> --output {output_dir}"
    for k, v in params.items():
...
def main():
    ...
    if args.refine:
        ...
        refined = refine_spatial_domain_identification(adata)
