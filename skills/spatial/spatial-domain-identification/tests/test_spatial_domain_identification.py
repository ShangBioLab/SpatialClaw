"""Tests for the spatial-domain-identification skill."""
...
SKILL_SCRIPT = Path(__file__).resolve().parent.parent / "spatial_domain_identification.py"
...
def test_demo_mode(tmp_output):
    """spatial-domain-identification --demo should run without error."""
...
def test_demo_result_json(tmp_output):
    ...
    assert data["skill"] == "spatial-domain-identification"
...
def test_demo_figures(tmp_output):
    ...
    assert (figures_dir / "spatial_domain_identification.png").exists()
...
def test_leiden_uses_prebuilt_graph():
    ...
    from skills.spatial._lib.domain_identification import identify_domains_leiden
...
def test_dispatch_invalid_method():
    ...
    from skills.spatial._lib.domain_identification import dispatch_method
...
def test_dispatch_supported_methods():
    ...
    from skills.spatial._lib.domain_identification import SUPPORTED_METHODS

    assert set(SUPPORTED_METHODS) == {"leiden", "louvain", "spagcn", "stagate", "graphst", "banksy"}


def test_refine_spatial_domain_identification():
    """Spatial refinement should produce labels for all cells."""
    from skills.spatial._lib.domain_identification import identify_domains_leiden, refine_spatial_domain_identification

    adata = _make_synthetic_adata()
    identify_domains_leiden(adata, resolution=0.5)
    refined = refine_spatial_domain_identification(adata, threshold=0.5, k=5)
