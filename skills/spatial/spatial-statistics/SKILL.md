...
# Demo mode
python skills/spatial/spatial-statistics/spatial_statistics.py --demo --output /tmp/spatial_stats_demo

# Via SPATIALCLAW runner
python spatialclaw.py run spatial-statistics --input <file> --output <dir>
python spatialclaw.py run spatial-statistics --demo
```
...
| `--output` | `results/spatial_statistics` | Output report directory |
| `--demo` | off | Run the built-in spatial statistics demo |
...
**Chaining**: Expects `processed.h5ad` from spatial-preprocessing as input
