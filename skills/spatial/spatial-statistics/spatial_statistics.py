...
def get_demo_data() -> tuple:
    ...
    with tempfile.TemporaryDirectory(prefix="spatialstats_demo_") as demo_dir:
        ...
        if result.returncode != 0:
            logger.error("spatial-preprocessing failed:\n%s", result.stderr)
            raise RuntimeError(f"spatial-preprocessing --demo failed (exit {result.returncode})")

        processed_path = demo_dir / "processed.h5ad"
        if not processed_path.exists():
            raise FileNotFoundError(
                f"Expected processed.h5ad at {processed_path} after spatial-preprocessing"
            )
