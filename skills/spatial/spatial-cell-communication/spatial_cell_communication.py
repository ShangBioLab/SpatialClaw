...
Supported methods:
  builtin      Curated ligand-receptor co-expression scoring (default)
  liana        LIANA+ multi-method consensus ranking
...
Usage:
    python spatial_cell_communication.py --input <preprocessed.h5ad> --output <dir>
    python spatial_cell_communication.py --input <data.h5ad> --output <dir> --method builtin
    python spatial_cell_communication.py --demo --output <dir>
"""
...
SKILL_NAME = "spatial-cell-communication"
SKILL_VERSION = "0.2.0"
...
def generate_figures(adata, output_dir: Path, summary: dict) -> list[str]:
    ...
    try:
        fig = plot_communication(adata, VizParams(), subtype="lr_ranked", top_n=20)
        p = save_figure(fig, output_dir, "lr_ranked.png")
        figures.append(str(p))
        plt.close('all')
    except Exception as exc:
        logger.warning("LR ranked plot failed: %s", exc)
    ...
    try:
        fig = plot_communication(adata, VizParams(), subtype="chord", top_n=20)
        p = save_figure(fig, output_dir, "lr_chord.png")
        figures.append(str(p))
        plt.close('all')
    except Exception as exc:
        logger.warning("LR chord plot failed: %s", exc)
...
def write_report(
    ...
    cmd = (
        f"python spatial_cell_communication.py --input <input.h5ad>"
    ...
    env_lines = []
    for pkg in ["scanpy", "anndata", "numpy", "pandas", "liana"]:
        try:
...
def get_demo_data() -> tuple:
    ...
    if not preprocess_script.exists():
        raise FileNotFoundError(f"spatial-preprocessing not found at {preprocess_script}")

    with tempfile.TemporaryDirectory(prefix="spatial_comm_demo_") as tmp_dir:
        tmp_path = Path(tmp_dir)
        logger.info("Generating demo data via spatial-preprocessing ...")
        ...
        # Inject realistic gene names so the built-in L-R scorer has overlap.
        valid_genes = [
            "ADIPOQ", "ADIPOR1", "ADIPOR2", "AGT", "AGTR1", "AGTR2",
            "ANGPT1", "ANGPT2", "TEK", "APLN", "APLNR", "ADM", "CALCRL",
            "ALCAM", "CD6", "ANXA1", "FPR1", "TGFB1", "TGFBR1", "TGFBR2",
            "VEGFA", "KDR", "FLT1", "CXCL12", "CXCR4", "CCL2", "CCR2",
            "IL6", "IL6R", "TNF", "TNFRSF1A", "EGF", "EGFR", "FGF2",
            "FGFR1", "PDGFA", "PDGFRA", "NOTCH1", "JAG1", "MIF", "CD74",
            "COL1A1", "ITGA1",
...
def main():
    parser = argparse.ArgumentParser(
        description="Spatial Communication — ligand-receptor interaction analysis\n"
                    "Built-in method is dependency-light; optional LIANA/CellPhoneDB/FastCCC methods require their packages.",
    ...
    parser.add_argument(
        "--method", default="builtin", choices=list(SUPPORTED_METHODS),
    )
