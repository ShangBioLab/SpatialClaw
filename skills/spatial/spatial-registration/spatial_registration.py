#!/usr/bin/env python3
"""Spatial registration for multi-slice alignment.

Usage:
    python spatial_registration.py --input <multi_slice.h5ad> --output <dir>
    python spatial_registration.py --input <data.h5ad> --output <dir> --reference-slice slice_1
    python spatial_registration.py --demo --output <dir>
"""
...
from skills.spatial._lib.registration import run_registration
from skills.spatial._lib.figure_io import save_figure

logging.basicConfig(level=logging.INFO, format="%(levelname)s: %(message)s")
logger = logging.getLogger(__name__)

SKILL_NAME = "spatial-registration"
SKILL_VERSION = "0.1.0"
...
def write_report(
    ...
    if summary.get("failed_slices"):
        body_lines.extend(["", "### Alignment Warnings\n"])
        for sl, reason in summary["failed_slices"].items():
            body_lines.append(f"- `{sl}`: {reason}")
        if summary.get("alignment_strategy") == "rigid_fallback":
            body_lines.append(
                "- PASTE was unavailable or incompatible for all non-reference slices; "
                "coordinates were aligned with deterministic rigid centering/scaling."
            )
    ...
    cmd_parts: list[str] = [f"python spatial_registration.py --input <input.h5ad> --output {output_dir}"]
    for k, v in params.items():
        if v is not None:
...
def get_demo_data() -> tuple:
    """Run spatial-preprocessing --demo and synthesize multi-slice data."""
    preprocess_script = (
    ...
    with tempfile.TemporaryDirectory(prefix="spatial_reg_demo_") as tmp_dir:
        ...
        logger.info("Running spatial-preprocessing --demo into %s", tmp_path)
        ...
        if result.returncode != 0:
            raise RuntimeError(
                f"spatial-preprocessing --demo failed (exit {result.returncode}):\n{result.stderr}"
            )
