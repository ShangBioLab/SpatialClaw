...
Filtering removes housekeeping genes and non-specific markers that pass statistical tests but lack biological specificity.
...
| Demo | n/a | `--demo` flag | Runs spatial-preprocessing demo first |

## Workflow

1. **Load**: Read preprocessed h5ad (output of spatial-preprocessing)
...
spatialclaw run spatial-de --demo --output /tmp/de_demo
...
|-----------|---------|-------------|
| `--input` | (required unless `--demo`) | Preprocessed AnnData file |
| `--output` | `results/spatial_de` | Output report directory |
...
**Chaining**: Expects `processed.h5ad` from spatial-preprocessing as input. Demo mode runs spatial-preprocessing automatically.
