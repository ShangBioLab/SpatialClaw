...
Routes natural language queries and file inputs to the correct SpatialClaw
...
KEYWORD_MAP: dict[str, str] = {
    ...
    "deepst": "spatial-modality-integrate",
    "pearlst": "spatial-modality-integrate",
    "spatial modality": "spatial-modality-integrate",
    "morphology": "spatial-modality-integrate",
    "histology image": "spatial-modality-integrate",
    "h&e image": "spatial-modality-integrate",
}
...
SKILL_DESCRIPTIONS: dict[str, str] = {
    ...
    "spatial-deconvolution": "Deconvolution — cell type proportions (Tangram, Stereoscope, GraphST)",
    "spatial-statistics": "Spatial statistics (Moran's I, Geary's C, Ripley, neighborhood enrichment)",
    "spatial-svg-detection": "Spatially variable genes (Moran's I, SpatialDE, FlashS)",
    ...
    "spatial-registration": "Spatial registration / slice alignment (PASTE)",
    "spatial-omics-integrate": "Spatial paired-modality integration (SpatialGlue, SpaDDM)",
    "spatial-modality-integrate": "Spatial domain analysis with DeepST or PearlST from directory inputs",
    "spatial-multi-sample-integration": "Multi-sample spatial integration and batch alignment",
    "spatial-visualization": "Spatial visualization and publication-ready plots",
    "spatial-orchestrator": "Spatial query routing and pipeline orchestration",
}
...
def _run_skill_subprocess(
    ...
    *,
    ...
    """Invoke spatialclaw.py run <skill> via subprocess."""
    spatialclaw_script = _PROJECT_ROOT / "spatialclaw.py"

    cmd = [sys.executable, str(spatialclaw_script), "run", skill_alias]
    if input_path:
...
def write_routing_report(
    ...
    body_lines.extend([
        "", "### Suggested Command\n",
        "```bash",
        f"python spatialclaw.py run {skill} --input <your_data.h5ad> --output /tmp/{skill}_output",
        "```",
    ])
...
def main():
    ...
    if args.list_skills:
        skills = list_skills()
        print(f"\nSpatialClaw Skills ({len(skills)} registered)\n")
    ...
    if args.query:
        ...
        print(f"\nSuggested command:")
        print(f"  python spatialclaw.py run {routing['skill']} \\")
