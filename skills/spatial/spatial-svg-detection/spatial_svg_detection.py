#!/usr/bin/env python3
"""Spatial SVG detection via multiple spatially variable gene methods.
...
Usage:
    python spatial_svg_detection.py --input <processed.h5ad> --output <dir>
    python spatial_svg_detection.py --demo --output <dir>
"""
...
from skills.spatial._lib.svg_detection import COUNT_BASED_METHODS, METHOD_DISPATCH, SUPPORTED_METHODS
...
SKILL_NAME = "spatial-svg-detection"
SKILL_VERSION = "0.2.0"
...
def write_report(output_dir: Path, svg_df: pd.DataFrame, summary: dict,
    ...
    cmd = f"python spatial_svg_detection.py --input <input.h5ad> --output {output_dir}"
    for k, v in params.items():
        cmd += f" --{k.replace('_', '-')} {v}"
...
def get_demo_data(output_dir: Path):
    ...
    with tempfile.TemporaryDirectory(prefix="svg_demo_") as tmpdir:
        ...
        if result.returncode != 0:
            raise RuntimeError(f"spatial-preprocessing --demo failed: {result.stderr}")
