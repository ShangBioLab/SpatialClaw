...
def run_de(
    ...
    # Validate log-normalized data for wilcoxon/t-test.  Dense numpy arrays also
    # expose ``.data`` as a memoryview, so only use ``.data`` for sparse arrays.
    X_data = adata.X.data if sparse.issparse(adata.X) else np.asarray(adata.X).ravel()
    if len(X_data) > 0:
