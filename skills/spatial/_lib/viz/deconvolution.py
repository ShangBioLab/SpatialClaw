...
- ``dominant``      — dominant cell type map
...
def _get_deconvolution_data(adata: Any, method: Optional[str]) -> DeconvolutionData:
    """Retrieve deconvolution proportions from adata.obsm."""
    available = []
    if "deconvolution_ct_pred" in adata.obsm:
        available.append("ct_pred")
    for key in adata.obsm.keys():
        if key == "deconvolution_ct_pred":
            continue
    ...
    if not available:
        raise ValueError(
            ...
            "Run spatial-deconvolution first."
        )
    ...
    return DeconvolutionData(proportions, method, cell_types, proportions_key)
    ...
    *,
    ...
    Args:
        ...
        subtype: ``"spatial_multi"`` (default), ``"dominant"``,
                 ``"diversity"``, or ``"umap"``.
        method: Deconvolution method key (e.g. ``"tangram"`` or ``"graphst"``).
    ...
    data = _get_deconvolution_data(adata, method)
...
def _plot_spatial_multi(adata: Any, params: VizParams, data: DeconvolutionData) -> plt.Figure:
    ...
    temp_key = "_deconvolution_viz_tmp"
    for i, ct in enumerate(cell_types):
        ax = axes[i]
        adata.obs[temp_key] = data.proportions[ct].values
        plot_spatial_feature(
            adata, ax=ax, feature=temp_key,
            params=VizParams(
...
def _plot_dominant(adata: Any, params: VizParams, data: DeconvolutionData) -> plt.Figure:
    """Dominant cell type at each spatial spot."""
...
def _plot_diversity(adata: Any, params: VizParams, data: DeconvolutionData) -> plt.Figure:
...
def _plot_umap_proportions(adata: Any, params: VizParams, data: DeconvolutionData) -> plt.Figure:
