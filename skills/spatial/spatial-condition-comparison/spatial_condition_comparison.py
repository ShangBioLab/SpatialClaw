#!/usr/bin/env python3
"""Spatial condition comparison with pseudobulk statistics.

Core analysis functions are in skills.spatial._lib.condition_comparison.

Usage:
    python spatial_condition_comparison.py --input <preprocessed.h5ad> --output <dir> --condition-key condition --sample-key sample_id
    python spatial_condition_comparison.py --demo --output <dir>
"""
...
from skills.spatial._lib.condition_comparison import run_condition_comparison, SUPPORTED_METHODS
from skills.spatial._lib.figure_io import save_figure

logging.basicConfig(level=logging.INFO, format="%(levelname)s: %(message)s")
logger = logging.getLogger(__name__)

SKILL_NAME = "spatial-condition-comparison"
SKILL_VERSION = "0.1.0"
...
def write_report(
    ...
    cmd = f"python spatial_condition_comparison.py --input <input.h5ad> --output {output_dir}"
    for k, v in params.items():
        if v is not None:
            cmd += f" --{k.replace('_', '-')} {v}"
