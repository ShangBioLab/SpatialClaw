...
def build_cli_alias_map() -> dict[str, str]:
    """Build {absolute_skill_dir_path: cli_alias} from the spatial registry."""
...
def generate_catalog() -> dict:
    """Scan spatial skills and build the catalog."""
    alias_map = build_cli_alias_map()
    skills = []
    for skill_md in sorted((SKILLS_DIR / "spatial").rglob("SKILL.md")):
        ...
        if isinstance(metadata, dict):
            sc_meta = metadata.get("spatialclaw", {}) or metadata.get("spatialclaw", {})
        ...
        entry = {
            ...
            "has_demo": has_demo,
            "demo_command": (
                f"python spatialclaw.py run {cli_alias or skill_dir.name} --demo"
