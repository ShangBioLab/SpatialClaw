"""SpatialClaw Textual TUI — full-screen terminal user interface.
...
if _HAS_TEXTUAL:
    class SpatialClawTUI(App):
        """SpatialClaw full-screen terminal chat interface."""
        ...
        def __init__(
            ...
            self._workspace = workspace_dir or str(_PROJECT_ROOT)
            self._model = model
        ...
        def compose(self) -> ComposeResult:
            with Horizontal(id="header-bar"):
                yield Label("⬡ SpatialClaw", id="header-title")
        ...
        async def _init_llm_async(self) -> None:
            try:
                ...
                model = os.environ.get("SPATIALCLAW_MODEL", self._config.get("model", ""))
                ...
                self._model = core.SPATIALCLAW_MODEL
                ...
                self._add_system_message(f"⚠ LLM init error: {e}\nRun 'python spatialclaw.py onboard' to configure.")
        ...
        async def _on_submit(self, event: Input.Submitted) -> None:
            ...
            if low in ("/exit", "/quit", "/q"):
                ...
                old_conversation_key = self._conversation_key()
                ...
                try:
                    import bot.core as core
                    core.conversations.pop(old_conversation_key, None)
                    core.reset_usage()
                except Exception:
                    pass
        ...
        async def _llm_response_async(self) -> None:
            ...
            try:
                ...
                core.conversations[conversation_key] = seed
                core._conversation_access[conversation_key] = _time.time()
                ...
                updated_msgs = list(core.conversations.get(conversation_key, []))
            ...
            if not candidate.exists():
                ...
                if found_builtin:
                    self._add_system_message(
                        ...
                        "Built-in skills are part of SpatialClaw core."
                    )
                else:
                    installed = (
        ...
        def action_clear_chat(self) -> None:
            ...
            try:
                import bot.core as core
                core.conversations.pop(conversation_key, None)
            except Exception:
                pass
            chat = self.query_one("#chat-area", ScrollableContainer)
        ...
        def action_show_help(self) -> None:
            self._add_system_message(
                "SpatialClaw TUI — Keyboard shortcuts:\n"
                "  Ctrl+N — New session\n"
                "  Ctrl+L — Clear chat\n"
...
def run_tui(
    ...
    """Launch the SpatialClaw Textual TUI application."""
    if not _HAS_TEXTUAL:
        raise ImportError(
