"""SpatialClaw interactive CLI — prompt_toolkit REPL loop.

Provides a rich terminal chat interface powered by SpatialClaw's existing
...
def print_banner(
    ...
    ws = workspace_dir or str(_PROJECT_ROOT)
...
def _handle_skills(arg: str) -> None:
    ...
    try:
        ...
        cli_app.list_skills(domain_filter=domain_filter)
...
def _handle_config(arg: str) -> None:
    ...
    if subcmd == "list":
        ...
        table = Table(title="SpatialClaw Configuration", show_header=True)
...
def _handle_install_skill(arg: str) -> None:
    ...
    if not script_candidates and not has_skill_md:
        console.print(
            ...
            "[dim]The skill may not be loadable. Make sure the directory follows SpatialClaw conventions.[/dim]"
        )
    else:
        console.print(
            f"[dim]Found:[/dim] "
...
async def _handle_research(arg: str, state: dict[str, Any]) -> None:
    ...
    try:
        from spatialclaw.agents import _check_research_deps
    ...
    try:
        from spatialclaw.agents.pipeline import ResearchPipeline
...
def _handle_uninstall_skill(arg: str) -> None:
    ...
    if not candidate.exists():
        ...
        if found_elsewhere:
            ...
            console.print("[dim]Built-in skills are part of the SpatialClaw core and should not be deleted.[/dim]")
        else:
...
def _init_llm(config: dict) -> tuple[str, str]:
    ...
    try:
        ...
        model = os.environ.get("SPATIALCLAW_MODEL", config.get("model", ""))
        base_url = os.environ.get("LLM_BASE_URL", config.get("base_url", ""))

        import logging
        logging.getLogger("spatialclaw.bot").setLevel(logging.WARNING)
        ...
        return core.SPATIALCLAW_MODEL, core.LLM_PROVIDER_NAME
    except Exception as e:
        logger.warning("LLM init error: %s", e)
        return os.environ.get("SPATIALCLAW_MODEL", "unknown"), os.environ.get("LLM_PROVIDER", "")
...
async def _stream_llm_response(
    ...
    *,
    ...
    try:
        ...
        core.conversations[chat_id] = seed
        core._conversation_access[chat_id] = __import__("time").time()
        ...
        updated_msgs = list(core.conversations.get(chat_id, []))
        messages.clear()
        ...
        if "api_key" in str(e).lower() or "authentication" in str(e).lower():
            console.print("[dim]Run [bold]spatialclaw onboard[/bold] to configure your API key.[/dim]")
        return err_msg
...
async def _async_interactive_loop(
    ...
    effective_workspace = workspace_dir or str(_PROJECT_ROOT)
    ...
    while state["running"]:
        try:
            ...
            if low in ("/exit", "/quit", "/q"):
                ...
                table = Table(title="SpatialClaw Commands", show_header=True, header_style="bold cyan")
...
def run_interactive(
    ...
    """Launch the interactive SpatialClaw CLI session.
...
async def _single_shot(
    ...
    await _stream_llm_response(messages, chat_id=session_id)

    await save_session(
        session_id, messages,
        model=resolved_model,
        workspace=workspace_dir or str(_PROJECT_ROOT),
    )
