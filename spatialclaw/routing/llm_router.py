...
def filter_cli_routing_skills(skills: Dict[str, str]) -> Dict[str, str]:
    """Remove Python API-only skills from CLI execution routing candidates."""
    return {
        name: desc
        for name, desc in skills.items()
        if name not in API_ONLY_SPATIAL_SKILLS
    }
...
def _resolve_llm_config() -> Tuple[str, str, str]:
    ...
    model = os.getenv("SPATIALCLAW_MODEL") or os.getenv("LLM_MODEL", "")
...
def route_with_llm(query: str, skills: Dict[str, str], domain: str) -> Tuple[Optional[str], float]:
    ...
    Args:
        ...
        domain: Spatial analysis domain label.

    Returns:
        (skill_name, confidence) tuple
    """
    api_key, base_url, model = _resolve_llm_config()
    skills = filter_cli_routing_skills(skills)
    if not skills:
        return None, 0.0
    ...
    prompt = f"""You are an expert in {domain} spatial analysis. Given a user query, select the MOST appropriate skill and provide a confidence score.
