...
Storage: one JSON file at `~/.config/spatialclaw/memory_snapshots/changeset.json`.
"""
...
def _default_snapshot_dir() -> str:
    env_dir = os.environ.get("SPATIALCLAW_SNAPSHOT_DIR")
    if env_dir:
        return env_dir
    return str(Path.home() / ".config" / "spatialclaw" / "memory_snapshots")
    ...
    def _load(self) -> Dict[str, Any]:
        ...
        if os.path.exists(p):
            try:
                ...
                logger.warning("ChangesetStore: failed to load %s, resetting snapshot state: %s", p, exc)
                return {"rows": {}}
            if "rows" not in data:
                return {"rows": {}}
            return data
