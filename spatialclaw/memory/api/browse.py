...
# Valid top-level browse domains. Typed memory categories such as preference,
# insight, dataset, and analysis live under episodic/semantic layer paths.
_DEFAULT_DOMAINS = "session,episodic,semantic"


def _get_valid_domains():
    raw = os.getenv("SPATIALCLAW_MEMORY_VALID_DOMAINS", _DEFAULT_DOMAINS)
...
class AddPathRequest(BaseModel):
    new_path: str
    target_path: str
    new_domain: str = "semantic"
    target_domain: str = "semantic"
    priority: int = 0
    disclosure: Optional[str] = None
...
async def get_node(
    ...
    domain: str = Query("session", description="Domain"),
):
