"""Custom backends for the SpatialClaw research pipeline.

Adapts EvoScientist's backend architecture:
- SpatialClawSandboxBackend: sandboxed shell execution + file ops
...
def create_sandbox_backend(workspace_dir: str):
    ...
    class SpatialClawSandboxBackend(LocalShellBackend):
        ...
        def __init__(self, root_dir: str, **kwargs):
            ...
            self._sandbox_id = f"spatialclaw-{uuid.uuid4().hex[:8]}"
    ...
    return SpatialClawSandboxBackend(workspace_dir)
...
def create_skills_backend(project_root: str):
    ...
    ----------
    project_root : str
        SpatialClaw project root (parent of skills/).
    ...
    class ReadOnlySkillsBackend(FilesystemBackend):
        """Read-only access to SpatialClaw skills definitions."""
