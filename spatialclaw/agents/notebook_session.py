"""Jupyter Notebook Session Manager for the SpatialClaw Research Pipeline.

Provides ``NotebookSession`` — a persistent Jupyter kernel backed
``.ipynb`` editing and execution engine.  Adapted from CellVoyager's
``NotebookSession`` design, tailored for SpatialClaw's spatial
...
from spatialclaw.paths import PROJECT_ROOT
...
# ---------------------------------------------------------------------------
# SpatialClaw project root (for PYTHONPATH injection into kernel)
# ---------------------------------------------------------------------------
...
class NotebookSession:
    ...
    ``adata``) survive between steps — enabling SpatialClaw skill functions
    ...
    def __init__(self, notebook_path: str, kernel_name: str = "python3"):
        ...
        bootstrap = (
            ...
            "\n"
            "# ── SpatialClaw skill loader ──────────────────────────────\n"
            "def load_skill(name):\n"
            "    '''Load an SpatialClaw skill module by name, e.g. load_skill(\"spatial-preprocessing\").\n"
            ...
            "    return mod\n"
            "\n"
            "print('✓ SpatialClaw kernel ready. Use load_skill(\"skill-name\") to load skills.')\n"
        )
