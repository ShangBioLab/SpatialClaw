        ...
        if hasattr(method, MAGIC_ATTR):
            ...
            def generate_function(name, key=key):
                def func(self, *args, **kwargs):
                    ...
                    worker = self._get_or_create_colocated_worker(key)
...
def create_colocated_worker_cls(class_dict: dict[str, RayClassWithInitArgs]):
    ...
    class WorkerDict(worker_cls):
        ...
        def _get_or_create_colocated_worker(self, key):
            worker = self.worker_dict.get(key)
            if worker is not None:
                return worker

            user_defined_cls = _unwrap_ray_remote(cls_dict[key])
            ...
            with patch.dict(os.environ, {"DISABLE_WORKER_INIT": "1"}):
                worker = user_defined_cls(*init_args_dict[key].get("args", ()), **init_args_dict[key].get("kwargs", {}))
