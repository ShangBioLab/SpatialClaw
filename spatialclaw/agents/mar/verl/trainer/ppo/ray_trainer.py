    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            if not self.config.recurrent.enable:
                ...
                output_gen_batch, final_mask, sample_index = self.generation_manager.run_llm_loop_revisit(test_gen_batch, {})
        ...
        metric_dict_path = os.path.join(self.config.trainer.default_local_dir.split('results')[0], 'log/val/', f"{self.config.trainer.experiment_name}.jsonl")
        if not os.path.exists(os.path.dirname(metric_dict_path)):
            os.makedirs(os.path.dirname(metric_dict_path))
        tmp_metric_dict_with_step = {"step": self.global_steps, **metric_dict}
        with open(metric_dict_path, "a+") as f:
            f.write(json.dumps(tmp_metric_dict_with_step) + "\n")
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                # with _timer('step', timing_raw):
                    ...
                    with _timer('adv', timing_raw):
                        ...
                        if not self.config.recurrent.enable:
                            ...
                            if self.use_rm:
                                raise NotImplementedError("RM is not implemented for recurrent.")
                            batch.batch['token_level_scores'] = reward_tensor[sample_index] # [num_actions]
                            # token_level_scores stores only the outcome-level rewards, no step-level rewards
                            ...
                            if self.config.algorithm.alpha != 1.0:
                                ...
                                all_recalled_memories_str = self.tokenizer.batch_decode(batch.batch['recalled_memories'], skip_special_tokens=True)
                                batch.batch['rewards_format'] = compute_format_rewards(all_responses_str, batch) # [num_actions]
                                ...
                                advantage_scalar = outcome_advantage_scalar * self.config.algorithm.alpha + \
                ...
                if metrics[save_score_key] > best_critic_score:
                    ...
                    if getattr(self.config.trainer, 'save_best_val', False) and self.global_steps > 20: # we skip the first 20 steps to avoid frequent saving
                ...
                metric_dict_path = os.path.join(self.config.trainer.default_local_dir.split('results')[0], 'log/train/', f"{self.config.trainer.experiment_name}.jsonl")
                if not os.path.exists(os.path.dirname(metric_dict_path)):
                    os.makedirs(os.path.dirname(metric_dict_path))
                tmp_metric_dict_with_step = {"step": self.global_steps, **metrics}
                tmp_metric_dict_with_step = {k: float(v) if isinstance(v, np.float64) else v for k, v in tmp_metric_dict_with_step.items()}
                with open(metric_dict_path, "a+") as f:
                    f.write(json.dumps(tmp_metric_dict_with_step) + "\n")
