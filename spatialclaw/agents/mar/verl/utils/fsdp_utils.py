...
def get_init_weight_context_manager(use_meta_tensor=True, mesh: DeviceMesh = None):
    ...
    try:
        ...
        init_empty_weights = accelerate_init_empty_weights
    except ImportError:
        pass

    if use_meta_tensor:
        if mesh is None:
            needs_meta_init = torch.distributed.get_rank() != 0
        else:
            needs_meta_init = mesh.get_coordinate()[-1] != 0
        if needs_meta_init:
            if init_empty_weights is None:
                raise ModuleNotFoundError(
                    "accelerate is required for meta-tensor initialization on non-source ranks, but it is not installed."
                )
            init_context = init_empty_weights
        else:
            if init_empty_weights is None:
                warnings.warn(
                    "accelerate is not installed; falling back to CPU weight initialization on the source rank.",
                    RuntimeWarning,
                )
