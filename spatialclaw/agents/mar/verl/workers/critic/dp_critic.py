...
def _require_flash_attn_bert_padding():
    if not FLASH_ATTN_BERT_PADDING_AVAILABLE:
        raise ModuleNotFoundError(
            "flash_attn is required when `use_remove_padding=True`, but it is not installed in the current environment."
        )
    ...
    def _forward_micro_batch(self, micro_batch):
        ...
        with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
            ...
            if self.use_remove_padding:
                _require_flash_attn_bert_padding()
