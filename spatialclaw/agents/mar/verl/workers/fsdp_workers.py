...
def _is_single_rank_process_group():
    if torch.distributed.is_initialized():
        return torch.distributed.get_world_size() <= 1
    return int(os.environ.get("WORLD_SIZE", "1")) <= 1


def _set_current_cuda_device():
...
def _init_process_group_with_explicit_device(backend="nccl"):
    backend = os.getenv("VERL_PROCESS_GROUP_BACKEND", backend).strip().lower()
    device_id = _set_current_cuda_device()
...
def _distributed_barrier():
    if not torch.distributed.is_initialized():
        return
    if _env_flag("VERL_SKIP_DISTRIBUTED_BARRIER", False):
        if int(os.environ.get("RANK", "0")) == 0:
            _log_single_rank_debug("distributed_barrier", skipped=True, reason="VERL_SKIP_DISTRIBUTED_BARRIER")
        return
    if _is_single_rank_process_group():
        return
...
def _log_single_rank_debug(tag, **kwargs):
    details = ", ".join(f"{k}={v}" for k, v in kwargs.items())
    print(f"[{tag}] {details}", flush=True)
...
class ActorRolloutRefWorker(Worker):
    ...
    def __init__(self, config: DictConfig, role: str):
        ...
        self.config = config
        self.role = role
        _log_single_rank_debug("worker_ctor", role=role, phase="enter", pid=os.getpid())
        import torch.distributed

        if not torch.distributed.is_initialized():
            _log_single_rank_debug("worker_ctor", role=role, phase="before_init_process_group")
            _init_process_group_with_explicit_device(backend="nccl")
        else:
            _set_current_cuda_device()
        _log_single_rank_debug(
            "worker_ctor",
            role=role,
            phase="after_init_process_group",
        ...
        _log_single_rank_debug("worker_ctor", role=role, phase="after_device_mesh", device_mesh=self.device_mesh)
    ...
    def _build_model_optimizer(
        ...
        if self.rank == 0:
            _log_single_rank_debug(f"fsdp:{role}:hf_loaded", world_size=torch.distributed.get_world_size(), attn_implementation=attn_implementation)
        ...
        ref_param_offload = self.config.ref.fsdp_config.get("param_offload", True)
        cpu_offload = None
        if role != "actor" and ref_param_offload and not _is_single_rank_process_group():
            cpu_offload = CPUOffload(offload_params=True)
        sync_module_states = not _is_single_rank_process_group()
        if self.rank == 0:
            _log_single_rank_debug(
                f"fsdp:{role}:wrap",
        ...
        if self.rank == 0:
            _log_single_rank_debug(f"fsdp:{role}:fsdp_ready", device=torch.cuda.current_device())
    ...
    def init_model(self):
        ...
        if self._is_ref:
            if self.rank == 0:
                _log_single_rank_debug("ref:init_model", phase="before_build_model")
