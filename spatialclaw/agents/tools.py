"""Agent-specific tools for the SpatialClaw research pipeline.

Provides:
- ``think_tool`` — structured reflection (from EvoScientist)
- ``tavily_search`` — web search via Tavily (from EvoScientist)
- ``spatialclaw_execute`` — run SpatialClaw analysis skills
...
def think_tool(reflection: str) -> str:
    ...
    When to use:
    - Before starting work: What do I know? What SpatialClaw skills are available?
    ...
    3. SpatialClaw skills — Is there a skill that fits the current task?
...
# =============================================================================
# SpatialClaw skill execution tool
# =============================================================================


@tool(parse_docstring=True)
def spatialclaw_execute(
    skill: str,
    input_path: str = "",
    output_dir: str = "",
    method: str = "",
    ...
    """Execute an SpatialClaw analysis skill.

    Runs a registered SpatialClaw spatial skill (e.g.,
    spatial-preprocessing or spatial-cell-annotation) with the specified
    parameters.

    Args:
        skill: Name of the SpatialClaw skill to run
        input_path: Path to input data file (h5ad, csv, etc.)
        output_dir: Directory for output files
        method: Analysis method to use (e.g., leiden, tangram, graphst)
    ...
    try:
        result = cli_app.run_skill(skill, **kwargs)
...
def skill_search(
    ...
    """Search SpatialClaw's skill registry for available analysis functions.

    Use this BEFORE writing notebook code to find if an existing SpatialClaw
    ...
    Args:
        ...
        domain: Optional domain filter. SpatialClaw currently exposes only
            the spatial domain; an empty value searches all spatial skills.
...
def notebook_create(
    ...
    The kernel has SpatialClaw's project root on PYTHONPATH, so skill
    ...
    raw_nb_path = _Path(notebook_path)
    if raw_nb_path.is_absolute():
        return "Error creating notebook: notebook_path must be relative to the workspace."

    workspace = _Path(_workspace_dir).resolve() if _workspace_dir else _Path.cwd().resolve()
    nb_path = (workspace / raw_nb_path).resolve()
    try:
        nb_path.relative_to(workspace)
    except ValueError:
        return "Error creating notebook: notebook_path cannot escape the workspace."
    ...
    try:
        ...
        return (
            ...
            f"  Kernel running (python3), PYTHONPATH includes SpatialClaw root.\n"
...
def notebook_add_execute(
    ...
    SpatialClaw skill functions directly through the notebook's preloaded
    ``load_skill()`` helper, e.g.::

        mod = load_skill("spatial-preprocessing")
...
def build_tool_registry() -> dict:
    ...
    return {
        ...
        "spatialclaw_execute": spatialclaw_execute,
