"""Tests for the MemoryClient high-level API (spatialclaw.memory.memory_client).
...
from spatialclaw.memory.memory_client import MemoryClient
from spatialclaw.memory.snapshot import ChangesetStore
...
async def test_boot_with_data(client, monkeypatch):
    ...
    monkeypatch.setenv("SPATIALCLAW_MEMORY_CORE_URIS", "project://identity")
...
def test_changeset_store_recovers_from_corrupt_json(tmp_path):
    store = ChangesetStore(str(tmp_path))
    corrupt_path = tmp_path / "changeset.json"
    corrupt_path.write_text('{"rows": {"broken": ', encoding="utf-8")

    data = store._load()

    assert data == {"rows": {}}

    store._save({"rows": {"ok": {"table": "nodes", "before": None, "after": None}}})
    repaired = store._load()
    assert "ok" in repaired["rows"]
