...
class TestAgentConfig:
    ...
    def test_config_yaml_exists(self):
        """config.yaml must exist in spatialclaw/agents/."""
        config_path = Path(__file__).parent.parent / "spatialclaw" / "agents" / "config.yaml"
...
class TestPrompts:
    ...
    def test_imports(self):
        ...
        from spatialclaw.agents.prompts import (
        ...
        assert "SpatialClaw" in RESEARCH_PIPELINE_WORKFLOW
        assert "reviewer" in REVIEWER_CHECKLIST.lower()

    def test_get_system_prompt(self):
        """get_system_prompt() returns a non-empty prompt."""
        from spatialclaw.agents.prompts import get_system_prompt
        prompt = get_system_prompt()
        assert len(prompt) > 100
        assert "SpatialClaw" in prompt

    def test_get_researcher_prompt_has_date(self):
        """get_researcher_prompt() includes current date."""
        from spatialclaw.agents.prompts import get_researcher_prompt
    ...
    def test_build_prompt_refs(self):
        ...
        from spatialclaw.agents.prompts import build_prompt_refs
...
class TestIntake:
    ...
    def test_intake_result_structure(self):
        ...
        from spatialclaw.agents.intake import IntakeResult
        result = IntakeResult()
    ...
    def test_extract_geo_accessions(self):
        ...
        from spatialclaw.agents.intake import _extract_geo_accessions
    ...
    def test_extract_organism_human(self):
        ...
        from spatialclaw.agents.intake import _extract_organism
    ...
    def test_extract_technology(self):
        ...
        from spatialclaw.agents.intake import _extract_technology
        assert _extract_technology("10x Visium spatial transcriptomics") == "10x Visium"
        # When multiple spatial techs are mentioned, all should be detected
        result = _extract_technology("Stereo-seq and MERFISH spatial assays")
        assert "Stereo-seq" in result
        assert "MERFISH" in result

    def test_extract_tissue(self):
        """Should detect tissue types."""
        from spatialclaw.agents.intake import _extract_tissue
    ...
    def test_pdf_to_markdown(self):
        ...
        from spatialclaw.agents.intake import _pdf_to_markdown
    ...
    def test_prepare_intake_mode_a(self):
        ...
        from spatialclaw.agents.intake import prepare_intake
...
class TestSlashCommand:
    ...
    def test_research_in_slash_commands(self):
        from spatialclaw.interactive._constants import SLASH_COMMANDS
