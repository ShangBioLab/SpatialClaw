"""
Telegram channel implementation for SpatialClaw.
...
logger = logging.getLogger("spatialclaw.channel.telegram")
...
class TelegramChannel(Channel):
    ...
    def run_polling(self) -> None:
        ...
        logger.info(
            f"Starting SpatialClaw Telegram bot "
            f"(provider: {core.LLM_PROVIDER_NAME}, model: {core.SPATIALCLAW_MODEL})"
        )
        logger.info(f"SpatialClaw directory: {core.PROJECT_ROOT}")
        ...
        print("SpatialClaw Telegram bot is running. Press Ctrl+C to stop.")
        try:
            loop.run_forever()
    ...
    async def _cmd_start(self, update, context) -> None:
        await update.message.reply_text(
            "Welcome to SpatialClaw -- spatial transcriptomics analysis at your fingertips!\n\n"
            "I can analyze spatial transcriptomics, histology-linked spatial data, "
            "and paired spatial modalities through a unified skill system.\n\n"
            "Commands:\n"
            "  /skills  -- list available analysis skills\n"
            "  /demo <skill>  -- run a demo (spatial-preprocessing, spatial-de, spatial-statistics, ...)\n"
            "  /status  -- bot info\n"
            "  /health  -- system health check\n\n"
            "Or just chat -- ask any spatial analysis question.\n"
            "Upload data files for personalized analysis.\n"
            "Send an image when you need tissue/platform identification.\n\n"
            "SpatialClaw is a research tool, not a medical device. "
    ...
    async def _cmd_demo(self, update, context) -> None:
        if not self._check_rate(update):
            return
        from bot import core
        skill = context.args[0] if context.args else "spatial-preprocessing"
        ...
        try:
            ...
            reply = await core.llm_tool_loop(
                ...
                f"Run the {skill} demo using the spatialclaw tool with mode='demo'.",
    ...
    async def _cmd_status(self, update, context) -> None:
        ...
        skills_dir = core.PROJECT_ROOT / "skills"
        skill_count = sum(
    ...
    async def _cmd_health(self, update, context) -> None:
        from bot import core
        checks = []
        if core.SPATIALCLAW_PY.exists():
            checks.append("SpatialClaw CLI: OK")
        else:
            checks.append("SpatialClaw CLI: MISSING")
            ...
            if caption:
                ...
                content_blocks.append({
                    ...
                    "text": (
                        ...
                        "transcriptomics platform. Then suggest which SpatialClaw analysis "
                        "skills would be appropriate (e.g. spatial-preprocessing, "
                        "spatial-domain-identification, spatial-cell-annotation). "
        ...
        try:
            ...
            try:
                upload_proc = await asyncio.create_subprocess_exec(
                    sys.executable, str(core.SPATIALCLAW_PY), "upload",
                    "--input", str(tmp_path),
