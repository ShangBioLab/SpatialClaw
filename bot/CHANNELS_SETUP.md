# SPATIALCLAW Channels Setup Guide

This document provides detailed, step-by-step configuration guides for all supported messaging platforms in SPATIALCLAW.
...
3. Follow prompts:
   - **Bot name**: `SPATIALCLAW` (or any display name)
   - **Bot username**: `SPATIALCLAW_bot` (must end with `_bot`, must be unique)
...
- `/setdescription @SPATIALCLAW_bot` -> "AI research assistant for spatial transcriptomics analysis. Supports spatial data, histology-linked spatial analysis, and paired spatial modalities."
- `/setabouttext @SPATIALCLAW_bot` -> "SPATIALCLAW - Your persistent AI research partner."
- `/setcommands @SPATIALCLAW_bot` -> 
  ```text
