"""
bot/onboard.py — Interactive Setup Wizard for SpatialClaw
"""
...
def run_onboard():
    console.print(Panel.fit("[bold cyan]SpatialClaw Configuration Wizard[/bold cyan]\nLet's get your AI spatial analysis assistant set up!", border_style="cyan"))
    ...
    if provider == "custom":
        ...
        model = questionary.text("Enter model name:", default=env_vars.get("SPATIALCLAW_MODEL", "")).ask()
        if model: env_vars["SPATIALCLAW_MODEL"] = model.strip()
    elif provider == "ollama":
        model = questionary.text("Enter Ollama model name (e.g. qwen2.5:7b):", default=env_vars.get("SPATIALCLAW_MODEL", "qwen2.5:7b")).ask()
        if model: env_vars["SPATIALCLAW_MODEL"] = model.strip()
