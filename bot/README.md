# SPATIALCLAW Messaging Bot System

The `bot` module powers SPATIALCLAW's conversational AI interfaces. It connects the core spatial skills engine with popular messaging platforms and a native terminal experience.
...
│         ↓                                                    │
│  ┌──────────────────────────────────────────┐                │
│  │  bot/core.py  (LLM tool loop)            │───▶ spatialclaw.py (skills)
│  └──────────────────────────────────────────┘                │
└──────────────────────────────────────────────────────────────┘
```
...
| `SPATIALCLAW_MODEL` | Override model name (optional, auto-set by provider) | All |
...
SPATIALCLAW supports **12 LLM providers** through a unified OpenAI-compatible interface:
...
| Custom endpoint | `custom` | *(set `SPATIALCLAW_MODEL`)* | *(set `LLM_API_KEY`)* |
...
SPATIALCLAW_MODEL=deepseek-reasoner   # Use R1 instead of default V3
```
...
| `/skills` | List all available SPATIALCLAW analysis skills |
...
Both platforms accept file uploads. The bot auto-detects spatial data formats and routes to the appropriate skill.
...
Bot:  (reads from NAS if SPATIALCLAW_DATA_DIRS includes /mnt/nas/spatial)
```
...
- Any additional paths in `SPATIALCLAW_DATA_DIRS`

To add external data directories (NAS, shared storage, other projects), set in `.env`:

```bash
SPATIALCLAW_DATA_DIRS=/mnt/nas/spatial_data,/home/user/experiments
```
...
- File paths are validated against a trusted directory whitelist (`data/`, `examples/`, `output/`, `SPATIALCLAW_DATA_DIRS`)
...
This is a known issue with `lark_oapi` — it uses a module-level event loop variable. SPATIALCLAW patches this automatically when running via `python -m bot.run`. If you see this error, ensure you're using the latest version of `bot/channels/feishu.py`.
