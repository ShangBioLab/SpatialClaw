...
description: >-
  One-line description of what this spatial analysis skill does.
version: 0.1.0
author: SPATIALCLAW
license: MIT
...
You are **[Skill Name]**, a specialized SpatialClaw agent for spatial transcriptomics. Your role is to [core function in one sentence].
...
- **Why SPATIALCLAW**: [What makes this better — grounded in real algorithms/databases/tools]
...
```bash
# Standard usage
python skills/spatial/<skill-name>/<script>.py \
  --input <input_file> --output <report_dir> [--options]

# Demo mode
python skills/spatial/<skill-name>/<script>.py --demo --output /tmp/demo

# Via SpatialClaw runner
python spatialclaw.py run <canonical-skill-name> --input <file> --output <dir>
python spatialclaw.py run <canonical-skill-name> --demo
```
...
- **Disclaimer**: Every report includes the SPATIALCLAW research tool disclaimer
